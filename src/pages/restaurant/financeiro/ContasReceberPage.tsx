...
import { FormField, TextInput, DateInput, SelectInput, CurrencyInput, PrimaryButton, SecondaryButton } from '../../../components/ui/FormComponents';
...
export const ContasReceberPage: React.FC = () => {
  ...
  const fetchContas = useCallback(async (isReset = true) => {
    ...
    try {
      const colRef = collection(db, 'restaurants', restaurantId, 'contasReceber');
      ...
      try {
        ...
        console.warn('[ContasReceber] Index missing fallback, querying by orderBy:', err);
      ...
      const newDocs = snap.docs.map(doc => ({ id: doc.id, ...doc.data() } as ContaReceber));
    ...
    <div className="space-y-6 max-w-7xl mx-auto pb-8">
      ...
      {loadingData ? (
        <div className="bg-white rounded-2xl border border-stone-200/80 p-8">
          <LoadingState message="Carregando contas a receber..." />
        </div>
            ...
            {filteredContas.map((conta) => (
              <div key={conta.id} className="bg-white p-4 rounded-2xl border border-stone-200/80 shadow-xs space-y-3">
                ...
                <div className="grid grid-cols-2 gap-2 text-xs">
                  ...
                  <div>
                    <span className="text-stone-400 block text-xs uppercase font-bold">Já Recebido</span>
      ...
      <div className="space-y-4">
        ...
        <div className="bg-stone-50 p-4 rounded-xl border border-stone-200/80 space-y-2">
          <div className="flex justify-between items-start">
            <div>
              <span className="text-xs font-bold text-stone-400 uppercase tracking-wider block">Cliente</span>
          ...
          <div className="grid grid-cols-3 gap-2 pt-2 border-t border-stone-200/60 text-xs">
            ...
            <div>
              <span className="text-stone-400 text-xs block font-bold uppercase">Já Recebido</span>
              <span className="font-bold text-emerald-600">{formatCurrency(conta.paidAmount / 100)}</span>
            </div>
            <div>
              <span className="text-amber-600 text-xs block font-bold uppercase">Saldo Restante</span>
